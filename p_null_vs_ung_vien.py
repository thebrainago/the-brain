...
def _bao_cao_md(tt: dict) -> str:
    ...
    return NL.join([
        ...
        "Moc so sanh cho van de `vd_p_ung_vien_lech_null`. Chuoi null duoc QUET",
        "cung mot luoi template/tham so da dang ky, roi lay p cua nhung o toi duoc",
        "tang placebo - tuc su chon loc duoc tinh vao ca hai ben.", "",
        "| Quan the | Tat ca o co p | Rieng nhom alpha > 0 |", "|---|---|---|",
        f"| CHUOI NULL | {_d(tt.get('tat_ca'))} | {_d(tt.get('alpha_duong'))} |",
        "| UNG VIEN the he cong 1 (15/08, n=692) | trung vi p=0.340 · "
        "p<0,05: 13.44% | alpha>0 chiem 41% |", "",
        f"Quet {tt.get('so_chuoi')} chuoi null, {tt.get('so_o')} o; "
        f"{tt.get('o_khong_toi_placebo')} o khong toi duoc tang placebo "
        f"(chet o cong re), {tt.get('so_loi')} loi.", "",
        "Doc: null cho ty le p<0,05 NGANG ung vien -> p lech he thong; null cho",
        "thap hon han -> phan du thua cua ung vien la that.",
    ])
